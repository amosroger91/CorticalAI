...
export default function ChatInterface() {
    ...
    const getMessageIcon = (msg) => {
        if (msg.type === 'user') return <PersonIcon sx={{ fontSize: 18 }} />;
        if (msg.type === 'function_result') return <CodeIcon sx={{ fontSize: 18 }} />;
        return <BotIcon sx={{ fontSize: 18 }} />;
    };

    const getMessageTitle = (msg) => {
        if (msg.type === 'user') return 'You';
        if (msg.type === 'function_result') return 'Function Result';
    ...
    const MarkdownComponents = {
        ...
        h1: ({ children }) => (
            <Typography variant="h4" gutterBottom sx={{ mt: 3, mb: 2, fontWeight: 700, letterSpacing: '-0.025em' }}>
                {children}
            </Typography>
        ),
        h2: ({ children }) => (
            <Typography variant="h5" gutterBottom sx={{ mt: 2.5, mb: 1.5, fontWeight: 600, letterSpacing: '-0.015em' }}>
                {children}
            </Typography>
        ),
        h3: ({ children }) => (
            <Typography variant="h6" gutterBottom sx={{ mt: 2, mb: 1.5, fontWeight: 600 }}>
                {children}
            </Typography>
        ),
        p: ({ children }) => (
            <Typography variant="body1" paragraph sx={{ mb: 1.5, lineHeight: 1.7 }}>
                {children}
            </Typography>
        ),
        a: ({ href, children }) => (
        ...
        >
            ...
            >
                <Toolbar sx={{ minHeight: '64px !important', px: 3 }}>
                    ...
                    {state.isStreaming ? (
                        <CircularProgress size={20} sx={{ color: state.config.primaryColor }} />
                    ) : (
            ...
            >
                ...
                {state.messages.map((msg, index) => (
                    <Slide
                        key={msg.id}
                        direction={msg.type === 'user' ? 'left' : 'right'}
                        in={true}
                        timeout={300}
                    >
                        <Box
            ...
            <Box sx={{ p: 3, pb: 4 }}>
                ...
                >
                    ...
                    {/* Enhanced Examples */}
                    {state.examples.length > 0 && state.messages.length === 0 && (
                        <Box sx={{ mt: 3, pt: 2 }}>
                            ...
                            <Grid container spacing={1.5}>
                                {state.examples.slice(0, 4).map((example, index) => (
                                    <Grid item key={index}>
                                        <Chip
                                            label={example}
